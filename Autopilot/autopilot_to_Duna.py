...
if theta0 > theta1:
    if math.pi >= theta1 >= need_theta:
        # Подходящий момент для запуска, нужно проверить, успеем ли мы включить двигатели
        delta_angle = theta1 - need_theta
        time_to_angle = delta_angle / w_rocket
        if time_to_angle - burn_time / 2 <= 0:
            # Момент упущен, ждём полный оборот
...
# Устанавливаем цель на Дюну
conn.space_center.target_body = Duna

print(f"Цель установлена на {conn.space_center.target_body.name}")
...
# print(f"Завершаем манёвр через {time_when_end - ut()} секунд")
